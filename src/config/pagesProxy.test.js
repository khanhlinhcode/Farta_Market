...
describe("Pages API proxy", () => {
  it("forwards cookies to the fixed API origin and preserves session cookies", async () => {
    ...
    const request = new Request("https://farta-storefront.pages.dev/api/me", {
      headers: { Cookie: "laravel_session=test", Referer: "https://farta-storefront.pages.dev/" },
    });
  ...
  it("rejects a cross-origin mutation before contacting the API", async () => {
    ...
    vi.stubGlobal("fetch", upstream);
    const request = new Request("https://farta-storefront.pages.dev/api/login", {
      method: "POST",
  ...
  it("adds HSTS to static asset responses", async () => {
    const assets = { fetch: vi.fn(async () => new Response("storefront")) };

    const response = await worker.fetch(
      new Request("https://farta-storefront.pages.dev/"),
      { ASSETS: assets }
    );
  ...
  it("keeps separate upstream cookies when adding response headers", async () => {
    ...
    const response = await worker.fetch(
      new Request("https://fartamarket.company/api/me"),
